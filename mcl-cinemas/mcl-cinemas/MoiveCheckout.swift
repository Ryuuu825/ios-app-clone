...
struct SessionDetailD : Codable {
    let n : String // date name
    let c : [SessionDetailP]
}

struct SessionDetailP : Codable {
    let ci : String // cinema id
    let cn : String // cinema name
    let o : Int
    let s : [SessionDetail]
}
...
struct SessionDetail : Codable {
    let si : Int // id?
    let sn : String // 星期二, 1月16日, 11:00 AM, 6院 $60
    let r : Int
...
struct DF {
    ...
    static func toDate(str: String, dateFor: String = "dd/mm") -> Date {
        let df = DateFormatter()
        df.dateFormat = dateFor
        df.locale = Locale(identifier: "zh-HK")
        return df.date(from: str)!
    }
    
    static func isSameDay(str1: String , str2: String, dateFor: String = "dd/mm" ) -> Bool {
        let d : Date = DF.toDate(str: str1, dateFor: dateFor)
        let d2 : Date  = DF.toDate(str: str2, dateFor: dateFor)
        return d == d2
    }
    
    
    
    static func isSameDay(d: Date , str: String, dateFor: String = "dd/mm" ) -> Bool {
        let d2 : Date  = DF.toDate(str: str, dateFor: dateFor)
        return d == d2
    }
                                                    ...
                                                    VStack(spacing: 4) {
                                                        ...
                                                        LazyVGrid( columns: Array(repeating: .init(.fixed(87)), count:4) , spacing: 12) {
                                                            ForEach(p.s , id:\.si) { ts in
                                                                ...
                                                                NavigationLink {
                                                                    SeatingPlan(movieId: movieId.description, purchase : .init(sessionDetail: .init(si: ts.si, sn: ts.sn, r: 92), cinemaName: p.cn, cinemaId: p.ci))
